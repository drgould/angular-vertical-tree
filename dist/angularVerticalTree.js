...
angular.module('drg.angularVerticalTree', []).directive('verticalTree', ["$compile", "$templateCache", "$timeout", function ($compile, $templateCache, $timeout) {
    return {
        restrict: 'EA',
        scope: {
            items: '<treeItems',
                ...
                post: function post(scope, elem) {
                    scope.updateBranchHeight = function () {
                        $timeout(function () {
                            ...
                            var breadcrumbsOuterHeight = breadcrumbs.toArray().reduce(function (height, breadcrumb) {
    ...
    function getCurrentPath() {
        if (opts.idProp) {
            return $scope.breadcrumbs.slice(1).map(function (breadcrumb) {
